'use client'

import { useEffect, useState, Suspense } from 'react'
import { useSearchParams } from 'next/navigation'
import Link from 'next/link'
import { Button } from '@/components/ui/button'
import { Card } from '@/components/ui/card'

function PedidoConfirmadoContent() {
  const searchParams = useSearchParams()
  const orderId = searchParams.get('orderId')
  const whatsappUrl = searchParams.get('whatsapp')
  const [countdown, setCountdown] = useState(5)

  useEffect(() => {
    if (whatsappUrl && countdown > 0) {
      const timer = setTimeout(() => setCountdown(countdown - 1), 1000)
      return () => clearTimeout(timer)
    } else if (whatsappUrl && countdown === 0) {
      window.location.href = decodeURIComponent(whatsappUrl)
    }
  }, [countdown, whatsappUrl])

  return (
    <div className="min-h-screen flex items-center justify-center p-8">
      <Card className="max-w-2xl w-full p-8 space-y-6 text-center animate-in fade-in zoom-in duration-500">
        <div className="mx-auto w-20 h-20 bg-green-100 rounded-full flex items-center justify-center">
          <svg className="w-10 h-10 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
          </svg>
        </div>

        <div className="space-y-2">
          <h1 className="font-bold text-3xl">Pedido Confirmado!</h1>
          <p className="text-muted-foreground text-lg">
            Seu pedido foi realizado com sucesso
          </p>
        </div>

        {orderId && (
          <Card className="bg-muted/50 p-4">
            <p className="text-sm text-muted-foreground">Número do pedido</p>
            <p className="font-mono font-bold text-lg">#{orderId.slice(0, 8).toUpperCase()}</p>
          </Card>
        )}

        <div className="space-y-4 pt-4">
          <p className="text-pretty leading-relaxed">
            Você será redirecionado para o WhatsApp para finalizar seu pedido com a vendedora em {countdown} segundos...
          </p>

          {whatsappUrl && (
            <Button asChild size="lg" className="w-full">
              <a href={decodeURIComponent(whatsappUrl)} target="_blank" rel="noopener noreferrer">
                <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.890-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
                </svg>
                Ir para WhatsApp Agora
              </a>
            </Button>
          )}

          <div className="flex flex-col sm:flex-row gap-3">
            <Button asChild variant="outline" className="flex-1">
              <Link href="/conta/pedidos">Ver Meus Pedidos</Link>
            </Button>
            <Button asChild variant="outline" className="flex-1">
              <Link href="/produtos">Continuar Comprando</Link>
            </Button>
          </div>
        </div>

        <div className="pt-4 space-y-2 text-sm text-muted-foreground">
          <p>Você receberá uma confirmação no WhatsApp</p>
          <p>Qualquer dúvida, entre em contato conosco</p>
        </div>
      </Card>
    </div>
  )
}

export default function PedidoConfirmadoPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen flex items-center justify-center">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary"></div>
      </div>
    }>
      <PedidoConfirmadoContent />
    </Suspense>
  )
}
